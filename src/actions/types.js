export const DRAW_LINE = "DRAW_LINE";
export const DRAW_FREEPATH = "DRAW_FREEPATH";
export const DRAW_RECTANGLE = "DRAW_RECTANGLE";
export const DRAW_ARROW = "DRAW_ARROW";
export const DRAW_CIRCLE = "DRAW_CIRCLE";
export const CLEAR_CANVAS_CLICK = "CLEAR_CANVAS_CLICK";
export const GET_CANVAS_DATA = "GET_CANVAS_DATA";
export const UPDATE_CANVAS_DATA = "UPDATE_CANVAS_DATA";
export const CLEAR_CANVAS_DATA = "CLEAR_CANVAS_DATA";
export const ERROR_FINDING_CANVAS_DATA = "ERROR_FINDING_CANVAS_DATA";
